...
const ROUTES = Object.freeze([
  ...
  Object.freeze({ label: "privacy-v3", path: "/legal/privacy/" }),
  Object.freeze({ label: "website-terms-v3", path: "/legal/website-terms/" }),
]);
...
function parseArguments(argv) {
  if (argv.length !== 2 || argv[0] !== "--finalized-artifact-root") {
    throw new Error("usage: browser-audit-joint-legal-v3.mjs --finalized-artifact-root /absolute/path");
  }
...
const profile = await import("node:fs/promises").then(({ mkdtemp }) =>
  mkdtemp(path.join(os.tmpdir(), "sitesourcery-joint-v3-browser-")));
...
try {
  ...
  console.log(`Joint legal V3 browser audit passed: ${ROUTES.length} routes × ${VIEWPORTS.length} viewports; loopback-only network; 9 screenshots.`);
} finally {
  if (cdp) cdp.close();
  await server.close();
