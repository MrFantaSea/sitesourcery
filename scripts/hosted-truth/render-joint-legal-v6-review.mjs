...
} from "./joint-legal-v6-review.mjs";

function parseArguments(argv) {
  if (argv.length !== 2 || argv[0] !== "--output" || !argv[1]) {
    throw new Error("usage: render-joint-legal-v6-review.mjs --output <new-directory>");
  }
  return argv[1];
}

export async function writeJointLegalV6Review({
  root = process.cwd(),
  output,
} = {}) {
  ...
  if (
    ...
    throw new Error("joint legal V6 review output must stay outside the repository");
  }
  const bundle = createJointLegalV6ReviewBundle({ root: absoluteRoot });
  ...
  await writeFile(
    path.join(absoluteOutput, "joint-legal-v6-review-manifest.json"),
...
if (
  ...
  writeJointLegalV6Review({ output: parseArguments(process.argv.slice(2)) })
    .then(({ output }) => {
      console.log(`Joint Legal V6 review rendered at ${output}`);
    })
    .catch((error) => {
