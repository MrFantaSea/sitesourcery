...
const DEFAULT_ARTIFACT_ROOT = path.join(ROOT, "_hosted");
const ARTIFACT_ROOT = path.resolve(
  process.env.SITESOURCERY_BROWSER_AUDIT_ARTIFACT_ROOT
...
if (process.env.SITESOURCERY_BROWSER_AUDIT_SKIP_BUILD === "1") {
  if (ARTIFACT_ROOT === path.resolve(DEFAULT_ARTIFACT_ROOT)) {
    throw new Error(
      "Skipping the browser-audit build requires an explicit external artifact.",
    );
  }
} else {
