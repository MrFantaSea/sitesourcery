...
test("hosted control mounts F03 only for active premium authority after offer release", async () => {
  ...
  assert.match(source, /var ALAKAZAM_PUBLIC_OFFER_STATE = "released";/u);
  assert.match(
    source,
  ...
  assert.match(
    source,
    /alakazam35Panel = windowRef\.SiteSourceryAlakazam35\.mount\(\{\s*documentRef,\s*container: alakazamPanel\.element,\s*projectId: selectedProjectId\s*\}\);/u
  );
  assert.match(
    source,
    /JSON\.stringify\(\[\s*accountId,\s*selectedProjectId,\s*tierId,\s*subscription\.status,\s*subscription\.revision\s*\]\)/u
  );
  assert.match(source, /alakazam35Panel\.destroy\(\)/u);
});
