...
test("quality and Pages retain history required for predecessor proofs", async () => {
  for (const [label, workflow] of [
    ["site quality", ".github/workflows/site-quality.yml"],
    ["Pages", ".github/workflows/pages.yml"],
  ]) {
    const source = await readFile(path.join(SITE_ROOT, workflow), "utf8");
    const checkoutAt = source.indexOf("- name: Check out repository");
    const setupAt = source.indexOf("- name: Use pinned Node.js", checkoutAt);
    assert.ok(checkoutAt >= 0, `${label} must check out the repository`);
    assert.ok(setupAt > checkoutAt, `${label} must set up Node after checkout`);
    assert.match(
      source.slice(checkoutAt, setupAt),
      /with:\s+fetch-depth:\s*0/u,
      `${label} must retain full history for production predecessor proofs`,
    );
  }
});
