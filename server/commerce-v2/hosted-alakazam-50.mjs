...
export function createHeldHostedAlakazam50() {
  async function held(actor) {
    requireActor(actor);
    throw new HostedError(
      "ALAKAZAM_50_HELD",
      "Alakazam $50 controls are held in this runtime.",
      { status: 503 }
    );
  }
  return Object.freeze({
...
export function createHostedAlakazam50({ controls, resolveSession } = {}) {
  invariant(
    controls &&
      ["configure", "read", "readiness", "requestCare"].every(
        (method) => typeof controls[method] === "function"
      ),
    "invalid_configuration",
    "the Alakazam $50 service is required",
    { status: 500 }
  );
  invariant(
    typeof resolveSession === "function",
    "invalid_configuration",
    "the Alakazam $50 scope resolver is required",
    { status: 500 }
  );
