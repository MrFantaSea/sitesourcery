...
function releasedJointLegalV7Authority() {
        ...
        select count(*) = 1
         ...
         where namespace.nspname = 'ss'
           ...
           and constraint_row.conname =
             'project_legal_acceptance_receipts_schema_version_v7_check'
           and pg_get_constraintdef(constraint_row.oid, false) =
             ...
             '''sitesourcery.project-legal-acceptance/v7''::text])))'
      ...
      (
        select count(*) = 1
         ...
         where namespace.nspname = 'ss'
           ...
           and constraint_row.conname =
             'customer_engagement_invitations_legal_schema_v7_check'
           and pg_get_constraintdef(constraint_row.oid, false) =
...
async function verifyJointLegalV7ReleaseState(pool) {
  const result = await pool.query(`
    select
      ss.hosted_joint_legal_v7_contract() =
        'canonical-hosted-joint-legal-v7-authority'
        as v7_contract_ready,
      (
        select count(*) = 3
          from ss.legal_documents document
         where (
           ...
           and document.kind = 'privacy'
           and document.version = 'SS-HOSTED-PRIVACY-2026-08-31-V7'
           and document.content_digest =
             '084788116b8d59f2e75faedd7cfad5ea14f007782c2a84679287f0d064753b99'
           and document.content_uri =
             'https://sitesourcery.com/legal/privacy/versions/SS-HOSTED-PRIVACY-2026-08-31-V7/'
        ...
        select count(*) = 1
         ...
         where namespace.nspname = 'ss'
           ...
           and pg_get_constraintdef(constraint_row.oid, false) =
             ...
             '''sitesourcery.project-legal-acceptance/v5''::text, ' ||
             '''sitesourcery.project-legal-acceptance/v7''::text])))'
      ) as engagement_v7_schema_ready
  `);
  for (const [name, ready] of Object.entries(result.rows[0])) {
    assert.equal(ready, true, `Joint legal V7 release state failed: ${name}`);
  }
}
...
async function verifyCustomerEngagementBootstrapJourney(pool) {
  ...
  const legalAuthority = releasedJointLegalV7Authority();
  const database = createCanonicalPostgresAuthority({ pool });
...
async function verifyAlakazamPolicyAuthority(pool) {
  ...
  const releasedPolicy = await pool.query(`
    select
      policy_id,
      policy_digest,
      policy_digest = ss.project_legal_json_digest(policy_document)
        as digest_ready,
      state = 'released'
        and commercial_effects
        and provider_effects
  ...
  `);
  assert.deepEqual(releasedPolicy.rows, [{
    policy_id: "SS-ALAKAZAM-POLICY-2026-08-31-V2",
    policy_digest:
      "145892e43ab6f4a03ebbed84fd148633f9a4de9727ce4294a0eb9b08f329c320",
    digest_ready: true,
    effects_released: true,
    customer_rights_ready: true,
    grants_ready: true,
    contract_ready: true
  }]);
  ...
  const releasedProjection = await pool.query(
    `select
       policy_id, authority_digest, state, hold_reason,
       commercial_effects, provider_effects, publication_effects,
       automatic_recovery_from_reversal_evidence,
  ...
  assert.deepEqual(releasedProjection.rows, [{
    policy_id: "SS-ALAKAZAM-POLICY-2026-08-31-V2",
    authority_digest:
      "145892e43ab6f4a03ebbed84fd148633f9a4de9727ce4294a0eb9b08f329c320",
    state: "released",
  ...
  assert.equal(readiness.state, "released");
  assert.equal(readiness.commercialEffects, true);
  assert.equal(readiness.providerEffects, true);
  assert.equal(readiness.publicationEffects, true);
  ...
  assert.equal(firstRepositoryRead.providerEffects, true);
...
export async function runMigrationVerification({
  ...
  try {
    ...
    await verifyJointLegalV7ReleaseState(pool);
