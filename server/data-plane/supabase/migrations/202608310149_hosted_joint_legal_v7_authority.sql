-- Migration 149: install the exact owner-approved Legal V7 authority
-- while retaining all V2/V3/V4/V5 documents, artifacts, receipts, and invitations.
begin;
...
begin
  ...
  then
    raise exception
      'Site Sourcery migrations through 148 must precede Legal V7 authority'
      using errcode = '55000';
  end if;
  ...
  if exists (
    select 1 from ss.legal_documents document
     where document.id in (
       ...
       '00000000-0000-4000-8000-000000000154'::uuid
     )
    ...
    raise exception 'Legal V7 identifiers must be unused before installation'
      using errcode = '55000';
  end if;
end
$$;

create temporary table hosted_joint_legal_v7_history_fingerprint
on commit drop
as
select
  (
...
create temporary table hosted_joint_legal_v7_release_constants (
...
-- Exact approved tuple from the retained V7 finalization receipt. It remains
-- held from public deployment even though database/runtime integration is exact.
insert into hosted_joint_legal_v7_release_constants (
  privacy_version,
  ...
  '2026-09-01T04:00:00.000Z'::timestamptz,
  24139,
  'https://sitesourcery.com/legal/privacy/versions/SS-HOSTED-PRIVACY-2026-08-31-V7/',
  'SS-HOSTED-WEBSITE-TERMS-2026-08-31-V7',
...
begin
  select * into strict release_record
    from hosted_joint_legal_v7_release_constants;
  if release_record.privacy_version <>
    ...
    or release_record.effective_at <>
      '2026-09-01T04:00:00.000Z'::timestamptz
    or release_record.privacy_byte_count <> 24139
    ...
    or release_record.website_terms_byte_count <> 27358
    or release_record.authority_digest !~ '^[a-f0-9]{64}$'
  then
    raise exception 'Hosted joint Legal V7 constants are invalid'
      using errcode = '55000';
  end if;
end
$$;
...
from hosted_joint_legal_v7_release_constants release
union all
select
  '00000000-0000-4000-8000-000000000153'::uuid,
  'product',
...
begin
  ...
  if (
    select count(*)
      from ss.legal_documents document
     where (
       document.id = '00000000-0000-4000-8000-000000000152'::uuid
       ...
       document.id = '00000000-0000-4000-8000-000000000153'::uuid
       ...
       document.id = '00000000-0000-4000-8000-000000000154'::uuid
    ...
    raise exception 'Hosted joint Legal V7 document postcondition failed'
      using errcode = '55000';
  end if;
  ...
  if (
    ...
    raise exception 'Hosted joint Legal V7 artifact postcondition failed'
      using errcode = '55000';
  end if;
  ...
  where privacy.id = '00000000-0000-4000-8000-000000000152'::uuid
    and product.id = '00000000-0000-4000-8000-000000000153'::uuid
    and website.id = '00000000-0000-4000-8000-000000000154'::uuid;

  if computed_authority_digest is null
    or computed_authority_digest <>
      release_record.authority_digest::ss.sha256_hex
  then
    raise exception 'Hosted joint Legal V7 authority digest changed'
      using errcode = '55000';
  end if;
end
$$;
...
alter table ss.project_legal_acceptance_receipts
  drop constraint project_legal_acceptance_receipts_schema_version_v5_check;

alter table ss.project_legal_acceptance_receipts
  add constraint project_legal_acceptance_receipts_schema_version_v7_check
  check (
    schema_version in (
...
alter table ss.customer_engagement_invitations
  drop constraint customer_engagement_invitations_legal_schema_v5_check;

alter table ss.customer_engagement_invitations
  add constraint customer_engagement_invitations_legal_schema_v7_check
  check (
...
begin
  ...
  then
    ...
    website_document_id :=
      '00000000-0000-4000-8000-000000000154'::uuid;
    authority_schema := 'sitesourcery.project-legal-authority/v7';
    privacy_pattern :=
      '^SS-HOSTED-PRIVACY-[0-9]{4}-[0-9]{2}-[0-9]{2}-V7$';
    website_pattern :=
      '^SS-HOSTED-WEBSITE-TERMS-[0-9]{4}-[0-9]{2}-[0-9]{2}-V7$';
  else
...
create function ss.hosted_joint_legal_v7_contract()
returns text
language sql
stable
...
revoke all on function ss.hosted_joint_legal_v7_contract()
from public, anon, authenticated, service_role;
grant execute on function ss.hosted_joint_legal_v7_contract()
to service_role;
...
begin
  select * into strict before_record
    from hosted_joint_legal_v7_history_fingerprint;
  ...
  then
    raise exception 'Legal V7 integration rewrote retained evidence'
      using errcode = '55000';
  end if;
end
$$;
