...
test("production F03 composition decorates fulfillment without changing the commercial enable predicate", async () => {
  ...
  assert.match(
    source,
    /createAlakazamFulfillmentWorker\(\{\s*repository: alakazamFulfillmentRepository,\s*compiler: alakazamTierCompiler,\s*publicationPort,[\s\S]*?enabled:\s*alakazamComposition\.mode === "approved" &&\s*publicationHeld\(\) === false,/u
  );
